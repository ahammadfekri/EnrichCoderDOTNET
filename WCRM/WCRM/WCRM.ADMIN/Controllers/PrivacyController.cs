...
namespace WCRM.ADMIN.Controllers
{
    ...
    {
        private readonly Privacy_DataAccess _dbHelper;
        private readonly IWebHostEnvironment _webHostEnvironment;
        public PrivacyController(Privacy_DataAccess dbHelper, IWebHostEnvironment webHostEnvironment)
        {
            _dbHelper = dbHelper;
        ...
        {
            List<Privacy> privacy = _dbHelper.GetAllPrivacies();
            return View(privacy);
        }
        ...
        [HttpPost]
        public IActionResult Create(Privacy model, IFormFile ImageFile)
        {
        ...
        {
            var privacies = _dbHelper.GetPrivacyById(id);
            if (privacies == null)
            {
                return NotFound();
            }
        ...
        [HttpPost]
        public IActionResult Edit(Privacy model, IFormFile ImageFile)
        {
            // Handle Image Upload
            if (ImageFile != null && ImageFile.Length > 0)
            {
            ...
            else
            {
                ...
                var existingPrivacy = _dbHelper.GetPrivacyById(model.Id);
                model.Icon = existingPrivacy.Icon;

            }
            if (model != null)
            {
                _dbHelper.UpdatePrivacy(model);
                return RedirectToAction("Index");
            }
            return View(model);
        }
